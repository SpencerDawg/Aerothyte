    ...
    public class GlacialHanger : ModItem
    {
        ...
        public override bool UseItem(Player player)
        {
            if (player.altFunctionUse == 2)
            {
                ...
                for (int i = 0; i < 4; i++)
                {
                    Projectile.NewProjectile(player.Center, player.DirectionTo(Main.MouseWorld).RotatedByRandom(MathHelper.ToRadians(30f)) * Main.rand.Next(4, 7), Mod.Get<ModProjectile>("GlacialHanger" + i).Type, 10, 5, player.whoAmI);
                }
            }
            return true;
        }
